"""
persistence/mempalace_adapter.py — Adaptador para memória semântica de longo prazo.

Encapsula todas as interações com o MemPalace, fornecendo uma interface
async-friendly para o bot Telegram. Renomeado de memory.py para evitar
conflito com módulo padrão do Python.
"""
...
async def mine_conversation(
    ...
    return await asyncio.to_thread(_mine_text_sync, text, wing, room, source)
...
def format_memories_for_context(memories: list[dict], max_tokens: int = 500) -> str:
    ...
    for i, mem_item in enumerate(memories, 1):
        similarity = mem_item.get("similarity", 0)
        text = mem_item.get("text", "").strip()
